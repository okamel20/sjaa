...
@section('script')
  ...
  <script type="text/javascript">
    ...
    $('.add').on('click',function(){ 
      window.location.href="{{ \URL::current().'/create?page_id='.request('page_id') }}";
    });
    ...
    $('.sendActive').on('click',function(){ 
      $('.form').prop('action','{{adminUrl('pagecontents/active/all')}}');
      setTimeout(function(){
            $('#form_active').submit();
      }, 500);
    });
    ...
    {
      $('.form').prop('action','{{adminUrl('pagecontents/destroy/all')}}');
      setTimeout(function(){
            $('#form_active').submit();
      }, 500);
